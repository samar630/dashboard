		...
		return (
			<div key={index}
				...
				<div className=" flex flex-col items-center text-teal-600">
					{/* <div className={`rounded-full transition duration-500 ease-in-out border-2 border-gray-300 h-12 w-12 flex items-center justify-center py-3  ${step.selected ? " bg-orange-500  text-white font-bold" : ""}`}>
						{step.completed ? <span className="text-white font-bold text-xl">✓</span> : index + 1}
					</div> */}
					<div className={`top-0  text-center mt-4 w-64 text-xs font-medium uppercase ${step.highlighted ? "text-gray-900" : "text-gray-400"}`}> {step.description}	</div>
				</div>
	...
	return (
        <div className='flex flex-col gap-16'>
          <div  className=" flex flex-row  w-full justify-between items-center" >
        	...
        	<div className="flex items-center  justify-center flex-col gap-[4rem] w-full h-full">
                <div className='p-8 rounded-lg border-[1px] border-gray-300' >
